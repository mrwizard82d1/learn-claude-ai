...
1. ✅ **DONE — `CLAUDE.md` box.** Personal `RPA/CLAUDE.local.md` imports `@_laj/rpa-architecture/README.md`; confirmed loaded (behavioral test — Claude recited executables / WCF `9296` / REST `7047` from memory, incl. deep specifics only the doc has). git-ignored via `.git/info/exclude`; zero team footprint. *Optional Step 2 (team citizenship):* PR a **lean Architecture section** into the team `CLAUDE.md` — **inline the text** (teammates lack the git-ignored `_laj` import target) or use a committed `.claude/rules/architecture.md`.
...
5. **Phase 2 next:** the `spec → plan` front half — a spec-driven exercise on an *ambiguous* problem (the kata's spec was given, so it didn't exercise this). **Then** the sequel: crystallize [`notes/tdd-with-agents.md`](./notes/tdd-with-agents.md) into a **personal `tdd-pairing` skill** (user-level; a "keep-me-honest" skill that enforces behavioral-first-red, minimal green, surprising-pass checks, and spec-ideas-as-hypotheses). Order stays: **practice → note → skill.**
...
- [x] Practice: real **CLAUDE.md** on RPA — authored an orientation doc (via `architecture-view` → `_laj/rpa-architecture/`) and wired it into RPA sessions via a personal **`CLAUDE.local.md`** `@import` (confirmed loaded via behavioral test — Claude recited the executables, WCF `9296`, REST `7047` from memory). No `/init` (RPA already has a team `CLAUDE.md`). *Optional Step 2:* PR a lean Architecture section into the team file. · ~0.5h
